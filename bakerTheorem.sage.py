...
_sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_6 = Integer(6); _sage_const_0p01 = RealNumber('0.01'); _sage_const_0p8 = RealNumber('0.8')
...
def bakerAuxiliaryFunction(L, n, h, D):
    ...
    f_AntiUnitaire = f.subs(c[L, L] == -_sage_const_1 )

    # 함수를 x에 대해 D번까지 미분합니다.
    df = [f_AntiUnitaire.diff(x, i) for i in range(D + _sage_const_1 )]
    ...
    def linear_equations_to_matrix(eqns, c_list):
        c_list= c_list[:-_sage_const_1 ]
        # 각 방정식을 재배열합니다.
        print([eq.lhs() for eq in eqns])
        v=[]
        for eq in eqns:
            substituting = dict(zip(c_list, [_sage_const_0 ] * len(c_list)))
    ...
    logsbyderivation = [(log(i))**y for i in A[_sage_const_1 ]]
    TwoDimensionalSolution = [{u[i]:solution[i]*logsbyderivation[i]} for i in range(len(u))]
    TwoDimensionalSolution.append({c[L,L]:(-_sage_const_1 )*(L*log(_sage_const_6 ))**y})
    # 해를 함수로 변환합니다.
...
if f_sol is not None:
    ...
    # 해를 함수로 변환합니다.
    df_sol = [f_sol.diff(x, i) for i in range(D + d + _sage_const_1 )]
    # print(latex(df_sol))
    print(
        "------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    )
    ...
    for i in range(D + d + _sage_const_1 ):
        for l in range(-d-_sage_const_1 , h + d+_sage_const_1 ):
    ...
    # 3D plot
    P = plot3d(fxy_sol(x=x,y=y), (x,-_sage_const_0p01 , h + _sage_const_0p01 ), (y, -d, D + d + _sage_const_1 ), color='purple', opacity=_sage_const_0p8 )
    P.show()
